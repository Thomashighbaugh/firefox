...
(function () {
  ...
  class VerticalTabsPaneBase {
    ...
    constructor() {
      ...
      this.innerBox = this.pane.appendChild(create(document, "vbox", { id: "vertical-tabs-inner-box" }));
      ...
      this.buttonsTabStop = this.buttonsRow.appendChild(create(document, "toolbartabstop", { "aria-hidden": true }));
      this.newTabButton = this.buttonsRow.appendChild(document.getElementById("new-tab-button").cloneNode(true));
      ...
      if (key_toggleVerticalTabs)
        this.closePaneButton.tooltipText += ` (${ShortcutUtils.prettifyShortcut(key_toggleVerticalTabs)})`;
      this.closePaneButton.addEventListener("command", (e) => this.toggle());
      this.innerBox.appendChild(create(document, "toolbarseparator"));
      this.scrollboxTabStop = this.innerBox.appendChild(create(document, "toolbartabstop", { "aria-hidden": true }));
      ...
      this.tabTooltip.setAttribute("onpopupshowing", `verticalTabsPane.createTabTooltip(event)`);
    ...
    get dragEvents() {
      return this._dragEvents || (this._dragEvents = ["dragstart", "dragleave", "dragover", "drop", "dragend"]);
    }
    ...
    get horizontalWalker() {
      ...
      return (this._horizontalWalker = document.createTreeWalker(this.pane, NodeFilter.SHOW_ELEMENT, (node) => {
    ...
    get verticalWalker() {
      ...
      return (this._verticalWalker = document.createTreeWalker(this.pane, NodeFilter.SHOW_ELEMENT, (node) => {
        if (node.tagName == "toolbartabstop") return NodeFilter.FILTER_ACCEPT;
        if (node.disabled || node.hidden) return NodeFilter.FILTER_REJECT;
        if (node.tagName == "button" || node.tagName == "toolbarbutton" || node.tagName == "checkbox") {
    ...
    async formatFluentStrings() {
      let [playingString, mutedString, blockedString, pipString] = await this.l10n.formatValues([
    ...
    resetPinnedTooltip() {
      ...
      this.pinPaneButton.tooltipText = config.l10n[newVal ? "Pin button tooltip" : "Collapse button tooltip"];
    }
    /**
    ...
    _populate() {
      ...
      for (let tab of gBrowser.tabs)
        if (this.filterFn(tab)) fragment[this.reversed ? `prepend` : `appendChild`](this._createRow(tab));
    ...
    _onFocus(e) {
      ...
      this.hoverOutQueued = false;
      this.hoverQueued = false;
      if (this.pane.getAttribute("unpinned") && !this.noExpand) this.pane.setAttribute("expanded", true);
    ...
    _onMouseEnter(e) {
      clearTimeout(this.hoverOutTimer);
      this.hoverOutQueued = false;
      if (!this.pane.getAttribute("unpinned") || this.noExpand) return this.pane.removeAttribute("expanded");
    ...
    unpin() {
      ...
      if (this.pane.matches(":hover, :focus-within") && !this.noExpand) this.pane.setAttribute("expanded", true);
    ...
    _onDragStart(e, tab) {
      ...
      tab._dragData = {
        movingTabs: (tab.multiselected ? gBrowser.selectedTabs : [tab]).filter(this.filterFn),
      };
      e.stopPropagation();
    }
    ...
    _onDragOver(e) {
      ...
      let getPosition = () => {
        return this.reversed ? row.tab._tPos < draggedTab._tPos : row.tab._tPos > draggedTab._tPos;
      };
...
#vertical-tabs-list .all-tabs-item .all-tabs-secondary-button[muted] {
    list-style-image: none !important;
    background-image: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="18px" height="18px" viewBox="0 0 18 18"><path fill-opacity="context-fill-opacity" fill="#f4f4f7" d="M3.52,5.367c-1.332,0-2.422,1.09-2.422,2.422v2.422c0,1.332,1.09,2.422,2.422,2.422h1.516l4.102,3.633V1.735L5.035,5.367H3.52z"/><path fill="#f4f4f7" fill-rule="evenodd" d="M12.155,12.066l-1.138-1.138l4.872-4.872l1.138,1.138 L12.155,12.066z"/><path fill="#f4f4f7" fill-rule="evenodd" d="M10.998,7.204l1.138-1.138l4.872,4.872l-1.138,1.138L10.998,7.204z"/></svg>') !important;
    transform: none !important;
    opacity: 0.7;
    margin-inline-start: -2px;
}
#vertical-tabs-list .all-tabs-item .all-tabs-secondary-button[activemedia-blocked] {
    list-style-image: none !important;
    background-image: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 12 12"><path fill-opacity="context-fill-opacity" fill="#f4f4f7" d="M2.128.13A.968.968 0 0 0 .676.964v10.068a.968.968 0 0 0 1.452.838l8.712-5.034a.968.968 0 0 0 0-1.676L2.128.13z"/></svg>') !important;
...
#vertical-tabs-list
    ...
    list-style-image: none !important;
    background-image: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 625.8 512"><path fill="#f4f4f7" fill-opacity="context-fill-opacity" d="M568.9 0h-512C25.6 0 0 25 0 56.3v398.8C0 486.4 25.6 512 56.9 512h512c31.3 0 56.9-25.6 56.9-56.9V56.3C625.8 25 600.2 0 568.9 0zm-512 425.7V86c0-16.5 13.5-30 30-30h452c16.5 0 30 13.5 30 30v339.6c0 16.5-13.5 30-30 30h-452c-16.5.1-30-13.4-30-29.9zM482 227.6H314.4c-16.5 0-30 13.5-30 30v110.7c0 16.5 13.5 30 30 30H482c16.5 0 30-13.5 30-30V257.6c0-16.5-13.5-30-30-30z"/></svg>') !important;
...
#vertical-tabs-pane[unpinned] .all-tabs-item[soundplaying] .sound-overlay {
    background: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="18px" height="18px" viewBox="0 0 18 18"><path fill-opacity="context-fill-opacity" fill="#f4f4f7" d="M3.52,5.367c-1.332,0-2.422,1.09-2.422,2.422v2.422c0,1.332,1.09,2.422,2.422,2.422h1.516l4.102,3.633 V1.735L5.035,5.367H3.52z M12.059,9c0-0.727-0.484-1.211-1.211-1.211v2.422C11.574,10.211,12.059,9.727,12.059,9z M14.48,9 c0-1.695-1.211-3.148-2.785-3.512l-0.363,1.09C12.422,6.82,13.27,7.789,13.27,9c0,1.211-0.848,2.18-1.938,2.422l0.484,1.09 C13.27,12.148,14.48,10.695,14.48,9z M12.543,3.188l-0.484,1.09C14.238,4.883,15.691,6.82,15.691,9c0,2.18-1.453,4.117-3.512,4.601 l0.484,1.09c2.422-0.605,4.238-2.906,4.238-5.691C16.902,6.215,15.086,3.914,12.543,3.188z"/></svg>')
        center/12px no-repeat;
}
#vertical-tabs-pane[unpinned] .all-tabs-item[muted] .sound-overlay {
    background: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="18px" height="18px" viewBox="0 0 18 18"><path fill-opacity="context-fill-opacity" fill="#f4f4f7" d="M3.52,5.367c-1.332,0-2.422,1.09-2.422,2.422v2.422c0,1.332,1.09,2.422,2.422,2.422h1.516l4.102,3.633V1.735L5.035,5.367H3.52z"/><path fill="#f4f4f7" fill-rule="evenodd" d="M12.155,12.066l-1.138-1.138l4.872-4.872l1.138,1.138 L12.155,12.066z"/><path fill="#f4f4f7" fill-rule="evenodd" d="M10.998,7.204l1.138-1.138l4.872,4.872l-1.138,1.138L10.998,7.204z"/></svg>')
        center/12px no-repeat;
}
#vertical-tabs-pane[unpinned] .all-tabs-item[activemedia-blocked] .sound-overlay {
    background: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16px" height="16px" viewBox="0 0 12 12" fill-opacity="context-fill-opacity" fill="#f4f4f7"><path d="M2.128.13A.968.968 0 0 0 .676.964v10.068a.968.968 0 0 0 1.452.838l8.712-5.034a.968.968 0 0 0 0-1.676L2.128.13z"/></svg>')
...
#vertical-tabs-button {
    list-style-image: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="context-fill %23f4f4f7"><path fill-opacity="context-fill-opacity" d="M2,7h3v6H2V7z"/><path d="M6,7v6H5V7H2V6h12v1H6z M13,1c1.657,0,3,1.343,3,3v8c0,1.657-1.343,3-3,3H3c-1.657,0-3-1.343-3-3V4c0-1.657,1.343-3,3-3H13z M3,3C2.448,3,2,3.448,2,4v8c0,0.6,0.4,1,1,1h10c0.6,0,1-0.4,1-1V4c0-0.6-0.4-1-1-1H3z"/></svg>');
    fill-opacity: 0.4;
}
...
#vertical-tabs-close-button {
    list-style-image: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="context-fill %23f4f4f7"><path fill-opacity="context-fill-opacity" d="M2,3h12v3H2V3z"/><path d="M6,7v6H5V7H2V6h12v1H6z M13,1c1.657,0,3,1.343,3,3v8c0,1.657-1.343,3-3,3H3c-1.657,0-3-1.343-3-3V4c0-1.657,1.343-3,3-3H13z M3,3C2.448,3,2,3.448,2,4v8c0,0.6,0.4,1,1,1h10c0.6,0,1-0.4,1-1V4c0-0.6-0.4-1-1-1H3z"/></svg>');
    fill-opacity: 0.4;
}
  ...
  function makeWidget() {
    ...
    CustomizableUI.createWidget({
      ...
      onCreated(node) {
        ...
        if (key_toggleVerticalTabs) node.tooltipText += ` (${ShortcutUtils.prettifyShortcut(key_toggleVerticalTabs)})`;
      },
    });
  }
