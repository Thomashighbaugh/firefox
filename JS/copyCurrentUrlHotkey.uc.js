  ...
  class CopyCurrentURL {
    ...
    get clipboardHelper() {
      return (
        ...
        (this._clipboardHelper = Cc["@mozilla.org/widget/clipboardhelper;1"].getService(Ci.nsIClipboardHelper))
      );
    }
    get contextMenu() {
    ...
    handleEvent(_e) {
      ...
      if (menu) menu.querySelector(`[cmd="cmd_copy"]`).setAttribute("key", CopyCurrentURL.config.shortcut.id);
      else this.setupHint();
    }
    setupHint() {
