...
(() => {
  ...
  function setUpScroll(oneOffs, mask = false) {
    ...
    container.frames = [
      {
        maskPositionX: "0px",
        maskSize: "100%",
        maskImage: "linear-gradient(to right, transparent 10px, rgb(0, 0, 0) 30px, rgb(0, 0, 0) 100%)",
      },
      {
        maskPositionX: "-30px",
        maskSize: "1000%",
    ...
    container.lineScrollAmount = function () {
      return buttonsList.length ? Math.round(buttons.scrollWidth * 0.1) / 0.1 / buttonsList.length : 30;
    };
    ...
    if (isSearchBar) {
      ...
      oneOffs.addEventListener("SelectedOneOffButtonChanged", oneOffs.on_SelectedOneOffButtonChanged, false);
    }
  }
