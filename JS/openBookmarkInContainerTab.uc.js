...
class OpenPlacesInContainerTabMenu {
  ...
  constructor() {
    ...
    let openPopup = this.placesMenuOpenInContainer.appendChild(document.createXULElement("menupopup"));
    ...
    let openAllPopup = this.placesMenuOpenAllInContainer.appendChild(document.createXULElement("menupopup"));
    ...
    let openAllLinksPopup = this.placesMenuOpenAllLinksInContainer.appendChild(document.createXULElement("menupopup"));
    openAllLinksPopup.addEventListener("command", (e) => this.openSelectedInContainer(e, openAllLinksPopup));
    ...
    if (location.href === `chrome://browser/content/browser.xhtml`) {
      ...
      let syncedOpenAllPopup = this.syncedMenuOpenAllInContainer.appendChild(document.createXULElement("menupopup"));
      syncedOpenAllPopup.addEventListener("command", (e) => this.openAllSyncedFromDevice(e, syncedOpenAllPopup));
      ...
      let syncedOpenPopup = this.syncedMenuOpenInContainer.appendChild(document.createXULElement("menupopup"));
  ...
  get syncedFilterContextMenu() {
    return (
      ...
      (this._syncedFilterContextMenu = document.getElementById("SyncedTabsSidebarTabsFilterContext"))
    );
  }
  get syncedTabsStore() {
  ...
  get placesMenuOpenNewTab() {
    return (
      this._placesMenuOpenNewTab || (this._placesMenuOpenNewTab = document.getElementById("placesContext_open:newtab"))
    );
  }
  get placesMenuCopy() {
    return this._placesMenuCopy || (this._placesMenuCopy = document.getElementById("placesContext_copy"));
  }
  ...
  get syncedMenuCopy() {
    return (
      this._syncedMenuCopy || (this._syncedMenuCopy = this.syncedContextMenu.querySelector("#syncedTabsCopySelected"))
    );
  }
  ...
  openSyncedTab(e, popup) {
    ...
    if (popup.triggerNode?.closest(".tabs-container")) this.openLinkInContainer(e, popup, this.selectedSyncedTab);
  }
  openAllSyncedFromDevice(e, popup) {
    if (!this.syncedContextMenuInited) return;
    if (popup.triggerNode?.closest(".tabs-container")) this.openLinkInContainer(e, popup, this.selectedSyncedRow.tabs);
  }
  loadSheet() {
...
if (location.href !== `chrome://browser/content/browser.xhtml` || gBrowserInit.delayedStartupFinished)
