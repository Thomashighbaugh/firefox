    ...
    gProtectionsHandler.handleProtectionsButtonEvent = function (event) {
      ...
      switch (event.type) {
        case "click":
          switch (event.button) {
            case 1:
              ...
              if (config["enable icon animation"]) {
                box.addEventListener("animationend", () => box.removeAttribute("animate"), { once: true });
          ...
          break;
        case "keypress":
          if (event.charCode != KeyEvent.DOM_VK_SPACE && event.keyCode != KeyEvent.DOM_VK_RETURN) return;
          break;
      }
