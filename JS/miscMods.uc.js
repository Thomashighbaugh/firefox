...
(function () {
  ...
  class UCMiscMods {
    constructor() {
      if (config["Disable bookmarks toolbar auto show"]) gEditItemOverlay._autoshowBookmarksToolbar = function () {};
      if (config["Moving tabs with arrow keys can wrap"]) gBrowser.arrowKeysShouldWrap = true;
      if (config["Stop downloads panel auto-focusing the footer button"]) this.stopDownloadsPanelFocus();
      ...
      if (config["Allow stop/reload button to animate in other toolbars"]) this.stopReloadAnimations();
      if (config["Fix missing tracking protection toast popup fade animation"]) this.fixProtectionsToast();
      if (config["Give the private browsing indicator a tooltip"]) this.addPrivateBrowsingTooltip();
      if (config["Preserve your default bookmarks folder"]) this.makeDefaultBookmarkFolderPermanent();
      if (config["Turn private browsing indicator into button"]) this.privateBrowsingIndicatorButton();
      if (config["Anchor permissions popup to granted permission icon"]) this.anchorPermissionsPopup();
      ...
      eval(
        `gProtectionsHandler.showProtectionsPopup = function ` +
          gProtectionsHandler.showProtectionsPopup
            .toSource()
            .replace(/PanelMultiView\.hidePopup\(this\.\_protectionsPopup\)/, `this._protectionsPopup.hidePopup(true)`)
      );
    }
    async addPrivateBrowsingTooltip() {
      ...
      document.querySelector(".private-browsing-indicator").tooltipText = await this.privateL10n.formatValue([l10nId]);
    }
    makeDefaultBookmarkFolderPermanent() {
      ...
      panel.addEventListener("popupshowing", (e) => {
        ...
        let pref = Services.prefs.getBoolPref("userChrome.bookmarks.editDialog.persistLastLocation", true);
      ...
      eval(
        `tabContainer.on_dragstart = function ` +
          tabContainer.on_dragstart
            ...
            .replace(/\"white\"/, `getComputedStyle(this).getPropertyValue("--in-content-bg-dark").trim() || "white"`)
      );
    }
    randomTinyStuff() {
      // give the tracking protection popup's info button a tooltip
      let etpPanel = document.getElementById("template-protections-popup")?.content.querySelector("#protections-popup");
      let setEtpPopupInfoTooltip = (e) => {
        ...
        } else if (infoButton.getAttribute("data-l10n-id")) return document.l10n.translateElements([infoButton]);
      ...
      SidebarUI.showSwitcherPanel = function () {
        ...
        this._switcherPanel.addEventListener("popuphiding", () => this._switcherTarget.classList.remove("active"), {
          once: true,
        });
