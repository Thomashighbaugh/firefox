    ...
    (win, key) => {
      if (win === window)
        Services.obs.notifyObservers(
          ...
          JSON.stringify([CustomizableUI.AREA_MENUBAR, AutoHideMenubar._node.getAttribute("inactive")])
        );
    }
  );
    ...
    window.onViewToolbarsPopupShowing = function (aEvent, aInsertPoint) {
      ...
      for (let toolbar of toolbarNodes) {
        ...
        if (toolbar.id == "PersonalToolbar") {
          ...
          if (toolbar.hasAttribute("key")) menuItem.setAttribute("key", toolbar.getAttribute("key"));
      ...
      if (toolbarItem && toolbarItem.localName == "toolbarpaletteitem") toolbarItem = toolbarItem.firstElementChild;
      ...
      for (let node of popup.querySelectorAll('menuitem[contexttype="toolbaritem"]')) node.hidden = showTabStripItems;
      for (let node of popup.querySelectorAll('menuitem[contexttype="tabbar"]')) node.hidden = !showTabStripItems;
      document
      ...
      document.getElementById("toolbarNavigatorItemsMenuSeparator").hidden = !showTabStripItems;
      if (!CustomizationHandler.isCustomizing() && CustomizableUI.isSpecialWidget(toolbarItem?.id || "")) {
      ...
      if (showTabStripItems) {
        ...
        document.getElementById("toolbar-context-undoCloseTab").disabled = SessionStore.getClosedTabCount(window) == 0;
        return;
      }
      let movable = toolbarItem && toolbarItem.id && CustomizableUI.isWidgetRemovable(toolbarItem);
      if (movable) {
        if (CustomizableUI.isSpecialWidget(toolbarItem.id)) moveToPanel.setAttribute("disabled", true);
