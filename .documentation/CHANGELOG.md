...
- **2026-06-26 12:00** [Documentation] JS/tab_groups/AGENTS.md: created subsystem documentation covering data model (GroupData, GroupStore), controllers (GroupManager, TabHooks, IconManager), UI (GroupTabItem, GroupContextMenu), management page (AboutGroups), and utilities (StoragePaths, IconRegistry) — architecture tree, data flow diagrams, conventions, and persistence details

- **2026-06-22 11:00** [Documentation] findVars.js and dump-selectors.js: rewritten as async IIFEs, write .txt files to disk via IOUtils (with nsIFileOutputStream + Blob download fallbacks), timestamped filenames, metadata header with per-source breakdown, configurable output path, shadow DOM traversal, deduplication; findVars.js now unions declared CSSOM variables with computed DOM variables for full coverage; dump-selectors.js separates CSSOM/DOM/inline sources and drops noisy `[attr="value"]` selectors by default
- **2026-06-22 10:30** [JS] AboutConfigMultipleSelection.uc.js: full modernization — IIFE wrap, "use strict", `let`/`const`, namespace under `window.AboutConfigMultiSelect`, unload cleanup, `addEventListener` instead of inline `oncommand`, English label "Copy as user_pref()", graceful fallback between legacy XUL tree and modern HTML table about:config, selection highlight CSS injection, button click exclusion guards, range selection via Shift+click
- **2026-05-13 12:00** [CSS/Chrome] Vertical tabs alignment: set titlebar-spacer to width:0/display:none in layout.css and vertical-tabs.css; stripped all nav-bar padding-inline, margin-inline, --toolbarbutton-outer-padding, --toolbarbutton-inner-padding, and button margins; set --tab-block-margin:0; removed vertical tab container top gap; zeroed new-tab-button margins/padding for perfect flush alignment with sidebar
