...
(function () {
  class FluentRevealEffect {
    ...
    constructor() {
      ...
      if (this._options.clickEffect) document.documentElement.setAttribute("fluent-reveal-click", true);
    }
    ...
    get toolbarButtons() {
      ...
      if (this._options.includeBookmarks)
        buttons = buttons.concat(Array.from(this.placesToolbarItems.querySelectorAll(".bookmark-item")));
      return buttons;
    }

    get placesToolbarItems() {
      return this._placesToolbarItems || (this._placesToolbarItems = document.getElementById("PlacesToolbarItems"));
    }
    ...
    handleEvent(e) {
      requestAnimationFrame((time) => {
        switch (e.type) {
          case "scroll":
          case "mousemove":
            if (this._options.clickEffect && this._options.is_pressed) this.generateEffectsForAll(e, true);
    ...
    generateToolbarButtonEffect(el, e, click = false) {
      ...
      if (areaStyle.display == "none" || areaStyle.visibility == "hidden" || areaStyle.visibility == "collapse") {
