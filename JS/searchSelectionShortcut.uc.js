...
(() => {
  function frameScript() {
    ...
    const { XPCOMUtils: XPCOMUtils } = ChromeUtils.import("resource://gre/modules/XPCOMUtils.jsm");
            ...
            if ("KeyF" === e.code && e.ctrlKey && e.shiftKey && !e.repeat) {
              try {
                ...
                if (s.text && !s.docSelectionIsCollapsed) {
                  let msg = {
                    csp: utils.E10SUtils.serializeCSP(e.composedTarget.ownerDocument.csp),
                    text: s.text,
                    linkURL: s.linkURL,
  ...
  function init() {
    try {
      messageManager.loadFrameScript(
        "data:application/javascript," + encodeURIComponent(`(${frameScript.toString()})()`),
        true
      );
    } catch (e) {}
    messageManager.addMessageListener("ctrl-shift-f", (message) => {
      if (message.target === gBrowser.selectedBrowser) {
        try {
          ...
          let where = new RegExp(`(${BROWSER_NEW_TAB_URL}|${HomePage.get(window)})`, "i").test(locationURL)
            ? "current"
            : "tab";
          ...
          if (/^((chrome|resource|file|moz-extension)\:\/\/|about:).+/.test(text)) {
            ...
            let fixup, fixable;
            try {
              fixup = Services.uriFixup.getFixupURIInfo(text, Services.uriFixup.FIXUP_FLAG_ALLOW_KEYWORD_LOOKUP);
              fixable = true;
            } catch (e) {
              fixable = false;
            }
            if (fixable && !fixup._keywordProviderName) {
              linkURL = fixup._fixedURI.scheme === "http" ? fixup._fixedURI.host : fixup._fixedURI.spec;
              if (linkURL) return openLinkIn(linkURL, where, options);
            }
          }

          BrowserSearch._loadSearch(text, where, false, "contextmenu", principal, csp, false);
        } catch (e) {}
      }
    });
  }
