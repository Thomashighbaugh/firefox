...
class PrivateTabManager {
  ...
  exec() {
    ...
    let openAllPrivate = _ucUtils.createElement(document, "menuitem", {
      ...
      oncommand: `event.userContextId = ${this.container.userContextId}; ${openAll.getAttribute("oncommand")}`,
      onclick: `event.userContextId = ${this.container.userContextId}; ${openAll.getAttribute("onclick")}`,
    });
    ...
    let openAllLinksPrivate = _ucUtils.createElement(document, "menuitem", {
      ...
      accesskey: "v",
      class: this.menuClass,
      oncommand: `event.userContextId = ${this.container.userContextId}; ${openAllLinks.getAttribute("oncommand")}`,
      onclick: `event.userContextId = ${this.container.userContextId}; ${openAllLinks.getAttribute("onclick")}`,
    });
    ...
    MozElements.MozTab.prototype.getAttribute = function (att) {
      if (att == "usercontextid" && this.isToggling) {
        ...
        return privateTab.orig_getAttribute.call(this, att) ? 0 : privateTab.container.userContextId;
      } else {
    ...
    Object.defineProperty(customElements.get("tabbrowser-tabs").prototype, "allTabs", {
      get: function allTabs() {
        ...
        while (children.length && children[children.length - 1].tagName != "tab") children.pop();
        return children;
      },
    });
    ...
    customElements.get("tabbrowser-tabs").prototype._updateNewTabVisibility = function () {
      ...
      if (adjacentNewTab && adjacentPrivateTab) {
        ...
        if (newTabFirst) doc.getElementById("tabs-newtab-button").after(doc.getElementById(privateTab.BTN2_ID));
  ...
  tabContext(_e) {
    document
      ...
      .setAttribute("checked", TabContextMenu.contextTab.userContextId == this.container.userContextId);
  }
  ...
  onWidgetAfterCreation(id) {
    if (id == this.BTN_ID) {
      ...
      if (newTabPlacement)
        CustomizableUI.addWidgetToArea(this.BTN_ID, CustomizableUI.AREA_TABSTRIP, newTabPlacement + 1);
      gBrowser.tabContainer._updateNewTabVisibility();
  ...
  get observePrivateTabs() {
    return (
      ...
      (this._observePrivateTabs = !this.config.neverClearData && !this.config.doNotClearDataUntilFxIsClosed)
    );
  }
