...
class TabContextMenuNavigation {
  ...
  // we want to disable/enable the back & forward menuitems just like the back/forward buttons in the navbar.
  // we also want to change the tooltips for all 4 menuitems based on whether more than 1 tab is selected.
  onPopupShowing(e) {
  ...
  loadStylesheet() {
    // we're gonna use a <style> element instead of the usual stylesheet service,
    // since this seems to be the only way to get media queries to work in an author sheet
    // without saving the stylesheet to disk somewhere and loading it from a chrome:// url.
    // this restricts us in some ways but it doesn't matter since these elements only appear in one place.
