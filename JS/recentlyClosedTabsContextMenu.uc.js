...
class UndoListInTabmenu {
  ...
  get shouldHideWindows() {
    ...
    return !windowData.length || (!this.config["Include popup windows"] && windowData.every((w) => w.isPopup));
  }
  ...
  get strings() {
    return this._strings || (this._strings = RecentlyClosedTabsAndWindowsMenuUtils.strings || this.generateStrings());
  }
  ...
  get closedTabsLabel() {
    return (
      ...
      (this._closedTabsLabel = this.strings.formatMessagesSync(["menu-history-undo-menu"])[0].attributes[0].value)
    );
  }
    ...
    context.addEventListener(
      ...
      (e) => {
        ...
        windowMenu.accessKey =
          ...
          (RTL_UI ? windowMenu.label.substr(0, 1) : winWords[winWords.length - 1]?.substr(0, 1) || "W");
  ...
  makeSidebarPopups(context) {
    ...
    this.sidebarTabMenu = this.create(doc, "menu", {
      ...
      accesskey:
        this.config.l10n["Tabs access key"] ||
        (RTL_UI ? this.closedTabsLabel.substr(0, 1) : tabWords[tabWords.length - 1]?.substr(0, 1) || "T"),
    });
    ...
    this.sidebarWindowMenu = this.create(doc, "menu", {
      ...
      accesskey:
        this.config.l10n["Windows access key"] ||
        (RTL_UI ? this.closedWindowsLabel.substr(0, 1) : winWords[winWords.length - 1]?.substr(0, 1) || "W"),
    });
  ...
  modMethods() {
    ...
    RecentlyClosedTabsAndWindowsMenuUtils.createEntry = function (
      ...
      if (aTagName == "menuitem") element.setAttribute("class", "menuitem-iconic bookmark-item menuitem-with-favicon");
      ...
      if (!forContext && aTagName != "menuitem") {
        ...
        if (aIndex == 0) element.setAttribute("key", "key_undoClose" + (aIsWindowsFragment ? "Window" : "Tab"));
      }
    ...
    RecentlyClosedTabsAndWindowsMenuUtils.createRestoreAllEntry = function (
      ...
      restoreAllElements.setAttribute(
        "oncommand",
        "for (var i = 0; i < " + aEntryCount + "; i++) undoClose" + (aIsWindowsFragment ? "Window" : "Tab") + "();"
      );
    ...
    RecentlyClosedTabsAndWindowsMenuUtils.getWindowsFragment = function (
      ...
      if (closedWindowData.length) {
        let menuLabelString =
          RecentlyClosedTabsAndWindowsMenuUtils.navigatorBundle.GetStringFromName("menuUndoCloseWindowLabel");
        ...
        for (let i = 0; i < closedWindowData.length; i++) {
          ...
          let label = otherTabsCount == 0 ? menuLabelStringSingleTab : PluralForm.get(otherTabsCount, menuLabelString);
          ...
          if (!undoItem.isPopup || UndoListInTabmenu.config["Include popup windows"] || !forContext)
...
class RecentlyClosedPanelContext {
  ...
  constructor() {
    ...
    window.undoCloseTab = function (index) {
      ...
      let tabsToRemove = index !== undefined ? [index] : new Array(SessionStore.getLastClosedTabCount(window)).fill(0);
  ...
  goBackOrHide(panelview, force = false) {
    ...
    let panel = multiView._panel || multiView;
    if (force) panel?.hidePopup(true);
    else if (multiView.openViews?.length > 2) multiView.goBack();
  }
  ...
  async forgetClosedWindow(e) {
    ...
    tabs.forEach(async (tab) => await this.forgetEntries(tab.entries));
    SessionStore.forgetClosedWindow(value);
    return true;
  }
  bookmarkFromTab(e) {
