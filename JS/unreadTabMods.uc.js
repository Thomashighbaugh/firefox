...
(function () {
  ...
  class UnreadTabsBase {
    ...
    static config = {
      "Add Context Menu Items": true, // if set to true, add "Mark Tab as Read" and "Mark Tab as Unread" to the tab context menu when appropriate.

      "Mark Tab as Read Label": "Mark Tab as Read", // the "Mark Tab as Read" context menu label when only one tab is invoked.

      "Mark Tab as Unread Label": "Mark Tab as Unread", // the "Mark Tab as Unread" context menu label when only one tab is invoked.

      "Mark Multiple Tabs as Read Label": "Mark #1 Tabs as Read", // when tabs are multiselected and the tab you right-clicked is one of the selected tabs, the context menu will mark multiple tabs instead of just one. make sure it still has "#1", that gets replaced dynamically with the number of tabs that will be marked. so if you're gonna change the label for your language, be sure to put #1 where the number should go. for example 見られるようにマーク#1タブ or Marquer #1 onglets comme lu.

      "Mark Multiple Tabs as Unread Label": "Mark #1 Tabs as Unread", // just like the one above but for marking tabs as unread.
    };
    constructor() {
      gBrowser.tabContainer._handleTabSelect = function (aInstant) {
        let selectedTab = this.selectedItem;
        if (this.getAttribute("overflow") == "true") this.arrowScrollbox.ensureElementIsVisible(selectedTab, aInstant);
    ...
    handleEvent(e) {
      ...
      this.markAsReadMenuitem.setAttribute(
        "label",
        unreadTabs.length > 1
          ? UnreadTabsBase.config["Mark Multiple Tabs as Read Label"].replace("#1", unreadTabs.length.toLocaleString())
          : UnreadTabsBase.config["Mark Tab as Read Label"]
      );
      this.markAsUnreadMenuitem.setAttribute(
        "label",
        readTabs.length > 1
          ? UnreadTabsBase.config["Mark Multiple Tabs as Unread Label"].replace("#1", readTabs.length.toLocaleString())
    ...
    unreadTabs(tabs) {
      return tabs.filter((aTab) => {
        return !(aTab.getAttribute("pending") || !aTab.getAttribute("notselectedsinceload") || aTab.selected);
      });
    }
    ...
    makeMenuItems(context) {
      ...
      this.markAsReadMenuitem.setAttribute("label", UnreadTabsBase.config["Mark Tab as Read Label"]);
      ...
      this.markAsUnreadMenuitem.setAttribute("label", UnreadTabsBase.config["Mark Tab as Unread Label"]);
