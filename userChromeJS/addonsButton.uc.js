...
UC.extensionOptionsMenu = {
  ...
  handleClick: function (e) {
    ...
    switch (e.button) {
      ...
      case 2:
        if (!hasMdf) {
          if (addon.userDisabled) addon.enable();
          else addon.disable();
    ...
    if (addon.operationsRequiringRestart) {
      if (toggling)
        if (addon.userDisabled)
          if (addon.isActive) cls.add("disabling");
          else cls.remove("enabling");
        else if (addon.isActive) cls.remove("disabling");
        else cls.add("enabling");
      else if (addon.userDisabled && addon.isActive) cls.add("disabling");
      else if (!addon.userDisabled && !addon.isActive) cls.add("enabling");
    } else {
      if (toggling) {
        if (addon.isActive) {
          if (addon.optionsURL) cls.remove("noOptions");
          cls.remove("disabled");
          cls.remove("disabling");
          cls.add("enabling");
        } else {
