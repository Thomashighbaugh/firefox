...
(() => {
  ...
  const config = {
    // SECTION:
    // when you type into the urlbar or open a new tab, the identity icon (lock icon)
    // is set to resemble a magnifying glass. historically my theme has replaced this
    // with a google logo, but I realized this is a bit invasive since many firefox users
    // avoid google like the plague. however, I dislike the search icon since it makes the urlbar
    // harder to distinguish from the searchbar. so I set up this module to dynamically check
    // the user's default search engine and look for a matching icon in the chrome/resources/engines/ folder.
    // if you use my theme, you'll already have this folder with several engine icons included in it.
    // if you don't use my theme, or if my theme doesn't include an icon for your search engine,
    // you can add one yourself by placing it in chrome/resources/engines/
    // your icon file must have an SVG, PNG, JPG, or JPEG file extension. use SVG for best visual results.
    // if other extensions work that I'm not aware of, make a request on https://github.com/aminomancer/uc.css.js/issues
    ...
    // delete all spaces but don't delete anything else. the engine name refers to whatever name appears
    // in the search shortcuts list in about:preferences#search (the table with engine names and keywords).
    // engine name is often the same as the name of the extension that created the engine, but not always.
    // so always check about:preferences#search to get the actual name of the engine.
    // keep in mind this feature is totally separate from the rest of the script,
    // which sets the identity icon in *one-off* search mode, e.g., after clicking a one-off search engine button.
    // this setting affects the normal search mode. if you set this setting to false,
    // it will leave the searchglass as-is in normal mode, but will still set icons in one-off search mode.
    // the script achieves this replacement by setting the value of a variable: --default-search-identity-icon
    // on its own, this doesn't do anything. if you don't have my theme duskFox, you'll need to add some CSS
    // to set the identity icon's URL to var(--default-search-identity-icon) as you normally would to replace an image.
    // I do this with the following CSS rule:
    ...
    //     list-style-image: var(--default-search-identity-icon, url("chrome://userchrome/content/search-glass.svg")) !important;
    // }
    "Try to replace searchglass icon with engine icon in normal mode": true,
    // !SECTION

    // SECTION:
    // by default, firefox ONLY shows your default search engine's name in the urlbar placeholder text IF
    // the engine is built into firefox, for example, the default Google engine.
    // if you switch your engine to an engine from an extension or one you built yourself
    // with "Add custom search engine", the placeholder will just say "Search or enter address".
    // this setting changes that. it will show the name in the placeholder even if the engine
    // was installed by the user. the extra settings below can add some restrictions if you want.
    "Show engine name in placeholder": true,
    // !SECTION

    // SECTION:
    // it's possible for an extension to add an engine with any arbitrary name.
    // so if the developer is stupid, they could name an engine "Awesome Amazon Search Extension OMGWTFBBQ"
    // and then you could end up with a placeholder that says "Search with Awesome Amazon Search Extension OMGWTFBBQ or enter address".
    // I imagine you can see the problem with that. so the following setting will limit the number of characters:
    // if the number of characters in the engine's name is above the character limit, it will use the generic placeholder instead.
    // I think 25 is a good default limit. if you don't want this limit at all,
    // i.e., you're okay with arbitrarily long engine names, set the value to -1 or 0
    "Engine name character limit": 25,
    // !SECTION

    // SECTION:
    // an engine name might also have an unreasonable number of words (too many spaces).
    // again, if the engine name is "Awesome Amazon Search Extension OMGWTFBBQ" it will go over the limit.
    // and the script will default to the generic placeholder "Search or enter address".
    // increase the word limit by changing the value 3 below. a value of -1 or 0 disables the limit entirely.
    ...
    window.findEngineIcon = function (name) {
      ...
      while (files.hasMoreElements()) {
        ...
        if (file.isFile() && fileParts && fileParts[1]) {
          ...
          if (joinedName.includes(fileParts[1]) || fileParts[1].includes(joinedName)) {
            filename = leafName;
          }
          if (!filename && fileParts[1].includes(nameParts[0])) filename = leafName;
        }
      }
      if (filename) return `url(chrome://userchrome/content/engines/${filename})`;
      return false;
    };
    ...
    async function searchModeCallback(mus, _observer) {
      for (let mu of mus) {
        ...
        if (mu.target === searchModeIndicatorFocused || mu.target === urlbar || buttons.contains(mu.target)) {
          ...
          if (!engineStr)
            switchTab =
              urlbar.getAttribute("actiontype") === "switchtab" && urlbar.getAttribute("actionoverride") !== "true";
