...
class TabTooltipNav {
  ...
  get tabBackForwardMenu() {
    return this._tabBackForwardMenu || (this._tabBackForwardMenu = document.querySelector("#tabBackForwardMenu"));
  }
  get tooltipBox() {
    return this._tooltipBox || (this._tooltipBox = this.navPopup.querySelector("#tab-nav-tooltip-box"));
  }
  get backButton() {
    return this._backButton || (this._backButton = this.navPopup.querySelector("#tab-nav-back"));
  }
  get forwardButton() {
    return this._forwardButton || (this._forwardButton = this.navPopup.querySelector("#tab-nav-forward"));
  }
  get reloadButton() {
    return this._reloadButton || (this._reloadButton = this.navPopup.querySelector("#tab-nav-reload"));
  }
  get favicon() {
    return this._favicon || (this._favicon = this.navPopup.querySelector("#tab-nav-tooltip-favicon"));
  }
  set knownWidth(val) {
    if (val) {
  ...
  constructor() {
    ...
    ["TabClose", "TabMove", "TabSelect", "TabAttrModified", "mousemove", "mouseleave", "mousedown"].forEach((ev) =>
  ...
  onPopupShowing(e) {
    ...
    this.backButton.tooltipText = multiselected ? l10n["Go Back (Multiselected)"] : l10n["Go Back (Single Tab)"];
    this.forwardButton.tooltipText = multiselected
      ? l10n["Go Forward (Multiselected)"]
      : l10n["Go Forward (Single Tab)"];
    this.reloadButton.tooltipText = multiselected ? l10n["Reload (Multiselected)"] : l10n["Reload (Single Tab)"];
  }
  onPopupShown(e) {
    this.isOpen = true;
  ...
  browserReloadWithFlags(tabs, flags) {
    ...
    tabs.forEach((tab) => {
      ...
      if (gBrowser.updateBrowserRemotenessByURL(browser, url)) {
        if (tab.linkedPanel) loadBrowserURI(browser, url, principal);
          ...
          tab.addEventListener("SSTabRestoring", () => loadBrowserURI(browser, url, principal), { once: true });
    ...
    function sendReloadMessage(tab) {
      tab.linkedBrowser.sendMessageToActor("Browser:Reload", { flags, handlingUserInput }, "BrowserTab");
    }
  }
  ...
  updateButtonsState(tabs = this.tabs) {
    this.backButton.disabled = !tabs.some((tab) => gBrowser.getBrowserForTab(tab).webNavigation?.canGoBack);
    this.forwardButton.disabled = !tabs.some((tab) => gBrowser.getBrowserForTab(tab).webNavigation?.canGoForward);
  }
  ...
  fillHistoryMenu(menupopup) {
    ...
    if (!menupopup.hasStatusListener && this.config["Update tooltip when hovering in the history menu"]) {
    ...
    function updateSessionHistory(sessionHistory, initial, ssInParent) {
      ...
      for (let j = end - 1; j >= start; j--) {
        ...
        let item = existingIndex < children.length ? children[existingIndex] : document.createXULElement("menuitem");
