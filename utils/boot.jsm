...
class ScriptData {
  ...
  get isEnabled() {
    return (yPref.get(PREF_SCRIPTSDISABLED) || "").split(",").indexOf(this.filename) === -1;
  }
  ...
  tryLoadIntoWindow(win) {
    ...
    try {
      ...
      Services.scriptloader.loadSubScriptWithOptions(`chrome://userscripts/content/${this.filename}`, {
        target: win,
  ...
  toggleScript: function (el) {
    ...
    if (script.isEnabled) {
      ...
      yPref.set(
        ...
        yPref.get(PREF_SCRIPTSDISABLED).replace(new RegExp(`^${script.filename},?|,${script.filename}`), "")
      );
    }
  ...
  startupFinished: function () {
    return new Promise((resolve) => {
      if (_ucjs.SESSION_RESTORED) {
        ...
        const obs_topic = APP_VARIANT.FIREFOX ? "sessionstore-windows-restored" : "mail-delayed-startup-finished";
  ...
  windowIsReady: function (win) {
    if (win && win.isChromeWindow) {
      return new Promise((resolve) => {
        ...
        const obs_topic = APP_VARIANT.FIREFOX ? "browser-delayed-startup-finished" : "mail-delayed-startup-finished";
    ...
    try {
      ...
      utils.windows.forEach((doc, win) => {
        ...
        let keyset =
          ...
          doc.body.appendChild(utils.createElement(doc, "keyset", { id: "ucKeys" }));
    ...
    buttons: [
      {
        ...
        callback: (notification) => {
          notification.ownerGlobal.openWebLinkIn("https://github.com/MrOtherGuy/fx-autoconfig#startup-error", "tab");
          return false;
        },
      },
    ],
...
UserChrome_js.prototype = {
  ...
  handleEvent: function (aEvent) {
    ...
    if (regex.test(window.location.href)) {
      ...
      const menu = document.querySelector(APP_VARIANT.FIREFOX ? "#menu_openDownloads" : "menuitem#addressBook");
      if (isWindow && menu) {
        ...
        document.l10n.formatValues(["restart-button-label", "clear-startup-cache-label"]).then((values) => {
