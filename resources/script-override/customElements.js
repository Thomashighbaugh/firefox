...
// This script can be loaded by componentHacker.uc.js to basically override any custom element in the chrome document.
// Normally we wouldn't be able to do this with custom elements loaded at this widget level, but if we create the
// custom element registry before the browser can, by using "initial-document-element-inserted" instead of
// "document-element-inserted" we can instantiate our own versions of the custom element before firefox can.
// I'm not currently using this because any changes I make to something so major would require frequent updating.
// I decided to figure out how to do this with autoconfig because mozilla decided to get rid of the little anchor arrows
// shown on arrow panels. That's what tells you what button the panel is relevant to. This seems crazy to me.
// They're called arrowpanels for fuck's sake. So I obviously wanted to revert this asinine change for change's sake.
// But it's just not practical. Keeping up with updates would be a full-time job. At that point I might as well fork the browser.
// I don't have the time or expertise to do that, so we will just have to put up with arrowless arrowpanels.
// Maybe some of you even hated the arrows — who knows.
(() => {
...
// restore the old arrow panels' arrows, for example.
{
